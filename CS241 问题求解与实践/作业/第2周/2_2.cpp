...
void  PolynomialDivision(Polynomial &a,Polynomial &b)
{
    double tmp1[a.RatioLength()], tmp2[b.RatioLength()];
    for(int i=0; i<a.RatioLength(); ++i)
        tmp1[i] = a.num(i);
    for(int i=0; i<b.RatioLength(); ++i)
        tmp2[i] = b.num(i);
    if (a.RatioLength() < b.RatioLength()) 
    {
        print4(0);
        cout << endl;
        a.simplify();
    }
    else if(b.RatioLength() == 1 && tmp2[0] == 0)
        cout <<"error" << endl << "error" << endl;
        else
        {
            int length = a.RatioLength()-b.RatioLength()+1;
            double quotient[length];
            for(int i=0; i<length; ++i)
            {
                
                quotient[length-i-1] = tmp1[a.RatioLength()-1-i]/tmp2[b.RatioLength()-1];
                
                tmp1[a.RatioLength()-1-i] = 0;
                for(int j=1; j<b.RatioLength(); ++j)
                tmp1[a.RatioLength()-1-i-j] -= quotient[length-i-1]*tmp2[b.RatioLength()-j-1];
            }
            Polynomial q(quotient, length), r(tmp1, b.RatioLength());
            q.simplify();
            r.simplify();
        }
}
...
int main()
{
    ...
    getline(cin, str1);
    getline(cin, str2);
    Polynomial a(str1),b(str2);
    PolynomialDivision(a, b);
}
