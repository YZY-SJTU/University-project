...
Polynomial(string str)
{
    ...
    {
        for(int i = 0;i<a.ratio.length();++i)
        ratio.calculate(i, a.ratio.search(i),'+');
        flag = a.flag;
    }
    void simplify()
    {
    ...
    Polynomial diff()
    {
        ...
        {
            double a[ratio.length()-1];
            for(int i=1; i<ratio.length(); ++i)
                    a[i-1] = i*ratio.search(i);
            Polynomial tmp(a, ratio.length()-1);
            return tmp;
        }
    }
...
bool Is0Point(Polynomial &tmp, float lh, float rh)
{
    double division = 0.000001,standard = 0.000001; 
    double present;
    bool flag = false;
    present = lh;
    while(present <= rh)
    {
        if(tmp.number(present) < standard && tmp.number(present) > -standard)
        {
            flag = true;
            break;
        }
...
bool IsChangeToken(Polynomial &tmp, float lh, float rh)
{
    double division = 0.000001; 
    double present;
    bool flag = false;
    int cnt1 = 0, cnt2 = 0;
    present = lh;
...
void ZeroPoint(Polynomial &tmp, float lh, float rh)
{
    Polynomial deriv1 = tmp.diff(), deriv2 = deriv1.diff();
    bool flag1 = Is0Point(deriv1, lh, rh), flag2 = IsChangeToken(deriv2, lh, rh); 
    if(lh >= rh ||tmp.number(lh) * tmp.number(rh) >= 0 || flag1 || flag2)
    {
        cout << "error" << endl;
        return;
    }
    if(lh - tmp.number(lh)/deriv1.number(lh) > rh && rh - tmp.number(rh)/deriv1.number(rh) < lh)
    {
        cout << "error" << endl;        
        return;
    }
    double standard = 0.000001,ZeroPoint = lh;
    while(tmp.number(ZeroPoint) > standard || tmp.number(ZeroPoint) < -standard)
            ZeroPoint -= tmp.number(ZeroPoint)/deriv1.number(ZeroPoint);
    print4(ZeroPoint);
    return;
}
...
int main()
{
    ...
    cin >> lh >> rh;
    Polynomial a(str);
    if(!a.IsRight()) cout << "error" << endl;
    else ZeroPoint(a, lh, rh);
    return 0;
}