	...
	if (n == 1)
	{
		...
		for (int i = 0; i < Node.size(); ++i)
		{
			...
			while (tmp != NULL)
			{
				if (tmp->end != i)
				{
					outfile << '<' << Node[i] << ' ' << fileName << "--subGraph" << ChangeToString(tmp->end + 1) << '.' << Node[tmp->end] << ' ' << tmp->weight << '>' << endl;
		...
		for (int j = 0; j < subGroup[i].size(); ++j)
		{
			...
			while (tmp != NULL)
			{
				...
				if (k == subGroup[i].size())
				{
					...
					subFile << '<' << Node[subGroup[i][j]] << ' ' << fileName << "--subGraph" << ChangeToString(endNodeInSubGraph + 1) << '.' << Node[tmp->end] << ' ' << tmp->weight << '>' << endl;
					totalWeight += tmp->weight;
				}
				tmp = tmp->next;
			}
...
//initialNode为初始的起点
void GraphGenerator::searchOneSubGraph(int NodeNum, string subGraphName, string mainFileName, vector<int>& reachNode, vector<searchRoot>& nextSearch, int initialNode)
{
...
vector<int> GraphGenerator::searchAllSubGraph(int NodeName, string fileName)
{
	//寻找结点是否在子图中存在
	...
	searchOneSubGraph(NodeName, subFileName, fileName, reachNode, nextSearch, NodeName);
	while (nextSearch.size() != 0)
	{
		vector<searchRoot> temp;
		for (int i = 0; i < nextSearch.size(); ++i)
			searchOneSubGraph(nextSearch[i].StartNodeNum, nextSearch[i].fileName, fileName, reachNode, temp, NodeName);
...
//输出-1表示打开出错
double GraphGenerator::searchFromFirstToSecond(int NodeName1, int NodeName2, string fileName, vector<int>& firstToSecond, vector<int>& result)
{
...
//节点出错时返回-2
//无道路时返回-1
double GraphGenerator::searchAllSubGraph(int NodeName1, int NodeName2, string fileName, vector<int> &result_1, double& weight1, vector<int> &result_2, double& weight2)
{
	bool AToB = false, BToA = false;
	weight1 = -1;
	weight2 = -1;
	vector<int> firstToSecond = searchAllSubGraph(NodeName1, fileName);
	if (firstToSecond.size() == 1 && firstToSecond[0] == -1)
	{
		cerr << "didn't find node" << endl;
		return -2;
	}
	vector<int> secondToFirst = searchAllSubGraph(NodeName2, fileName);
	...
	if (AToB && BToA)
	{
		weight1 = searchFromFirstToSecond(NodeName1, NodeName2, fileName, firstToSecond, result_1);
		weight2 = searchFromFirstToSecond(NodeName2, NodeName1, fileName, secondToFirst, result_2);
		if (weight1 > weight2)
			return weight2;
		else return weight1;
	}
