                       ...
                       float3 shadowFactor)
{
    ...
    //triangle index within the geometry object.
    ...
    //给出了现在的TMax，可由此计算出当前hit position
    //defines the current TMax point along the ray according to the following formula: Origin + (Direction * CommittedRayT).
    //query.CommittedRayT();
    //Given attributes a0, a1 and a2 for the 3 vertices of a triangle
    //返回一个float2 float2.x is the weight for a1 and float2.y is the weight for a2
    //a = a0 + float2.x \* (a1-a0) + float2.y\* (a2 -- a0).
    //query.CommittedTriangleBarycentrics();
    //返回一个bool值
    //TRUE means front face, FALSE means back face.
