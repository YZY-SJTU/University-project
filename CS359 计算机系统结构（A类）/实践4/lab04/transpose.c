...
void transpose_blocking(int n, int blocksize, int *dst, int *src) {
	...
	for(int xblock = 0; xblock < cnt; xblock++){
		for(int yblock = 0; yblock < cnt; yblock++){
			for(int x = 0; x < blocksize && xblock * blocksize + x < n; x++){
				for(int y = 0; y < blocksize && yblock * blocksize + y < n; y++){
					dst[(yblock * blocksize + y) * n + xblock * blocksize + x] = src[(xblock * blocksize + x) * n+ yblock * blocksize + y];
				}			
			}
		}	
	}
